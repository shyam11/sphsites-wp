		...
		<div id="review_form_wrapper">
			<div id="review_form">
				...
				$comment_form = array(
					...
					'fields'               => array(
						'author' => '<p class="comment-form-author">' . "\n" . 
							...
							'<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="35" required />' . "\n" . 
						'</p>' . "\n",
						'email'  => '<p class="comment-form-email">' . "\n" . 
							'<label for="email">' . esc_html__('Email', 'medical-clinic') . '<span class="cmsmasters_req">*</span>' . '</label>' . "\n" . 
							'<input id="email" name="email" type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="35" required />' . "\n" . 
						'</p>' . "\n",
					),
				...
				if ( $account_page_url ) {
					...
					$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( esc_html__( 'You must be %slogged in%S to post a review.', 'medical-clinic' ), '<a href="' . esc_url( $account_page_url ) . '">', '</a>' ) . '</p>';
				}

				if ( $ratings_enabled ) {
					$comment_form['comment_field'] = '<p class="comment-form-rating"><label for="rating">' . esc_html__( 'Your Rating', 'medical-clinic' ) . '</label><select name="rating" id="rating" required>
				...
				$comment_form['comment_field'] .= '<p class="comment-form-comment">' . 
					...
					'<textarea id="comment" name="comment" cols="67" rows="2" required></textarea>' . 
				'</p>';
